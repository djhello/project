    ...
    public class dbResistor
    {
        ...
        public async Task<List<vmResistor>> getAll()
        {
            ...
            try
            {
                using (_ctx)
                {
                    Resistors = await (from cp in _ctx.dsResistor
        ...
        public async Task<vmResistor> getById(int id)
        {
            ...
            try
            {
                using (_ctx)
                {
                    model = await (from cp in _ctx.dsResistor
                                                join p in _ctx.dsProject on cp.ProjectId equals p.Id
                                                join l in _ctx.dsLocation on cp.LocationId equals l.Id
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            //Update Resistor
                            var entityUpdate = _ctx.dsResistor.FirstOrDefault(x => x.Id == model.Id);
                        ...
                        else
                        {
                           var ResistorModel = new Resistor
                           {
                            ...
                            _ctx.dsResistor.Add(ResistorModel);
                            await _ctx.SaveChangesAsync();
                        }
        ...
        public async Task<string> deleteById(int id)
        {
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        var idToRemove = _ctx.dsResistor.SingleOrDefault(x => x.Id == id);
                        if (idToRemove != null)
                        {
                            _ctx.dsResistor.Remove(idToRemove);
                            await _ctx.SaveChangesAsync();
                        }
                        _ctxTransaction.Commit();
                        message = MessageConstants.Deleted;
