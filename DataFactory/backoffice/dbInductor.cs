    ...
    public class dbInductor
    {
        ...
        public async Task<List<vmInductor>> getAll()
        {
            ...
            try
            {
                using (_ctx)
                {
                    Inductors = await (from cp in _ctx.dsInductor
        ...
        public async Task<vmInductor> getById(int id)
        {
            ...
            try
            {
                using (_ctx)
                {
                    model = await (from cp in _ctx.dsInductor
                                   join p in _ctx.dsProject on cp.ProjectId equals p.Id
                                                join l in _ctx.dsLocation on cp.LocationId equals l.Id
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            //Update Inductor
                            var entityUpdate = _ctx.dsInductor.FirstOrDefault(x => x.Id == model.Id);
                        ...
                        else
                        {
                           var InductorModel = new Inductor
                           {
                            ...
                            _ctx.dsInductor.Add(InductorModel);
                            await _ctx.SaveChangesAsync();
                        }
        ...
        public async Task<string> deleteById(int id)
        {
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        var idToRemove = _ctx.dsInductor.SingleOrDefault(x => x.Id == id);
                        if (idToRemove != null)
                        {
                            _ctx.dsInductor.Remove(idToRemove);
                            await _ctx.SaveChangesAsync();
                        }
                        _ctxTransaction.Commit();
                        message = MessageConstants.Deleted;
