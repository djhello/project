    ...
    public class dbOpamp
    {
        ...
        public async Task<List<vmOpamp>> getAll()
        {
            ...
            try
            {
                using (_ctx)
                {
                    Opamps = await (from cp in _ctx.dsOpamp
        ...
        public async Task<vmOpamp> getById(int id)
        {
            ...
            try
            {
                using (_ctx)
                {
                    model = await (from cp in _ctx.dsOpamp
                                   join p in _ctx.dsProject on cp.ProjectId equals p.Id
                                                join l in _ctx.dsLocation on cp.LocationId equals l.Id
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            //Update Capacitor
                            var entityUpdate = _ctx.dsOpamp.FirstOrDefault(x => x.Id == model.Id);
        ...
        public async Task<string> deleteById(int id)
        {
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        var idToRemove = _ctx.dsOpamp.SingleOrDefault(x => x.Id == id);
                        if (idToRemove != null)
                        {
                            _ctx.dsOpamp.Remove(idToRemove);
                            await _ctx.SaveChangesAsync();
                        }
                        _ctxTransaction.Commit();
                        message = MessageConstants.Deleted;
