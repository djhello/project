...
{
    public class dbICregulator
    {
        private EquipmentDBContext _ctx = null;

        public dbICregulator()
        {
        ...
        public async Task<List<vmICregulator>> getAll()
        {
            ...
            try
            {
                using (_ctx)
                {
                    capacitors = await (from cp in _ctx.dsICregulator
        ...
        public async Task<vmICregulator> getById(int id)
        {
            ...
            try
            {
                using (_ctx)
                {
                    model = await (from cp in _ctx.dsICregulator
                                   join p in _ctx.dsProject on cp.ProjectId equals p.Id
                                                join l in _ctx.dsLocation on cp.LocationId equals l.Id
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            //Update Capacitor
                            var entityUpdate = _ctx.dsICregulator.FirstOrDefault(x => x.Id == model.Id);
                        ...
                        else
                        {
                           var capacitorModel = new ICregulator
                           {
                            ...
                            _ctx.dsICregulator.Add(capacitorModel);
                            await _ctx.SaveChangesAsync();
                        }
        ...
        public async Task<string> deleteById(int id)
        {
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        var idToRemove = _ctx.dsICregulator.SingleOrDefault(x => x.Id == id);
                        if (idToRemove != null)
                        {
                            _ctx.dsICregulator.Remove(idToRemove);
                            await _ctx.SaveChangesAsync();
                        }
                        _ctxTransaction.Commit();
                        message = MessageConstants.Deleted;
