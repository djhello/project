    ...
    public class dbDiode
    {
        ...
        public async Task<List<vmDiode>> getAll()
        {
            ...
            try
            {
                using (_ctx)
                {
                    diodes = await (from cp in _ctx.dsDiode
        ...
        public async Task<vmDiode> getById(int id)
        {
            ...
            try
            {
                using (_ctx)
                {
                    model = await (from cp in _ctx.dsDiode
                                                join p in _ctx.dsProject on cp.ProjectId equals p.Id
                                                join l in _ctx.dsLocation on cp.LocationId equals l.Id
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            //Update Capacitor
                            var entityUpdate = _ctx.dsDiode.FirstOrDefault(x => x.Id == model.Id);
        ...
        public async Task<string> deleteById(int id)
        {
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        var idToRemove = _ctx.dsDiode.SingleOrDefault(x => x.Id == id);
                        if (idToRemove != null)
                        {
                            _ctx.dsDiode.Remove(idToRemove);
                            await _ctx.SaveChangesAsync();
                        }
                        _ctxTransaction.Commit();
                        message = MessageConstants.Deleted;
