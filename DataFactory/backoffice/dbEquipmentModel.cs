...
{
    public class dbEquipmentModel
    {
        private EquipmentDBContext _ctx = null;

        public dbEquipmentModel()
        {
        ...
        public async Task<List<vmEquipmentModel>> getAll()
        {
            ...
            try
            {
                using (_ctx)
                {
                    equipmentModels = await (from eqM in _ctx.dsEquipmentModel
                                             join d in _ctx.dsDepartman on eqM.DepartmanId equals d.DepartmanId
                                             where eqM.Status==1
        ...
        public async Task<EquipmentModel> getById(int id)
        {
            ...
            try
            {
                using (_ctx)
                {
                    model = await _ctx.dsEquipmentModel.FirstOrDefaultAsync(x => x.Id == id);
                }
            }
            catch (Exception ex)
            {
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            //Update calibration
                            var entityUpdate = _ctx.dsEquipmentModel.FirstOrDefault(x => x.Id == model.Id);
                        ...
                        else
                        {
                           var equipmentModel = new EquipmentModel
                            ...
                            _ctx.dsEquipmentModel.Add(equipmentModel);
                            await _ctx.SaveChangesAsync();
                        }
        ...
        public async Task<string> deleteById(int id)
        {
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        var idToRemove = _ctx.dsEquipmentModel.SingleOrDefault(x => x.Id == id);
                        if (idToRemove != null)
                        {
                            _ctx.dsEquipmentModel.Remove(idToRemove);
                            await _ctx.SaveChangesAsync();
                        }
                        _ctxTransaction.Commit();
                        message = MessageConstants.Deleted;
