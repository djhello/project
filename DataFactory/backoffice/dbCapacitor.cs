    ...
    public class dbCapacitor
    {
        ...
        public async Task<List<vmCapacitor>> getAll()
        {
            ...
            try
            {
                using (_ctx)
                {
                    vCapacitors = await (from cp in _ctx.dsCapacitor
        ...
        public async Task<vmCapacitor> getById(int id)
        {
            ...
            try
            {
                using (_ctx)
                {
                    model = await (from cp in _ctx.dsCapacitor
                                                join p in _ctx.dsProject on cp.ProjectId equals p.Id
                                                join l in _ctx.dsLocation on cp.LocationId equals l.Id
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            //Update Capacitor
                               var entityUpdate = _ctx.dsCapacitor.FirstOrDefault(x => x.Id == model.Id);
                        ...
                        else
                        {
                            ...
                            _ctx.dsCapacitor.Add(capacitorModel);
                            await _ctx.SaveChangesAsync();
                        }
        ...
        public async Task<string> deleteById(int id)
        {
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        var idToRemove = _ctx.dsCapacitor.SingleOrDefault(x => x.Id == id);
                        if (idToRemove != null)
                        {
                            _ctx.dsCapacitor.Remove(idToRemove);
                            await _ctx.SaveChangesAsync();
                        }
                        _ctxTransaction.Commit();
                        message = MessageConstants.Deleted;
