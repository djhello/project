    ...
    public class OtherICs
    {
        ...
        public async Task<List<vmOtherIC>> getall()
        {
            ...
            try
            {
                using (_ctx)
                {
                    OtherICs = await (from cp in _ctx.OtherIC
        ...
        public async Task<vmOtherIC> getbyid(int id)
        {
            ...
            try
            {
                using (_ctx)
                {
                    model = await (from cp in _ctx.OtherIC
                                                join p in _ctx.Project on cp.ProjectId equals p.Id
                                                join l in _ctx.Location on cp.LocationId equals l.Id
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            //Update OtherIC
                            var entityUpdate = _ctx.OtherIC.FirstOrDefault(x => x.Id == model.Id);
        ...
        public async Task<string> deletebyid(int id)
        {
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        var idToRemove = _ctx.OtherIC.SingleOrDefault(x => x.Id == id);
                        if (idToRemove != null)
                        {
                            _ctx.OtherIC.Remove(idToRemove);
                            await _ctx.SaveChangesAsync();
                        }
                        _ctxTransaction.Commit();
                        message = MessageConstants.Deleted;
