    ...
    public class Relays
    {
        ...
        public async Task<List<vmRelay>> getall()
        {
            ...
            try
            {
                using (_ctx)
                {
                    Relays = await (from cp in _ctx.Relay
        ...
        public async Task<vmRelay> getbyid(int id)
        {
            ...
            try
            {
                using (_ctx)
                {
                    model = await (from cp in _ctx.Relay
                                   join p in _ctx.Project on cp.ProjectId equals p.Id
                                                join l in _ctx.Location on cp.LocationId equals l.Id
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            //Update Relay
                            var entityUpdate = _ctx.Relay.FirstOrDefault(x => x.Id == model.Id);
        ...
        public async Task<string> deletebyid(int id)
        {
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        var idToRemove = _ctx.Relay.SingleOrDefault(x => x.Id == id);
                        if (idToRemove != null)
                        {
                            _ctx.Relay.Remove(idToRemove);
                            await _ctx.SaveChangesAsync();
                        }
                        _ctxTransaction.Commit();
                        message = MessageConstants.Deleted;
