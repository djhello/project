    ...
    public class Mosfets
    {
        ...
        public async Task<List<vmMosfet>> getall()
        {
            ...
            try
            {
                using (_ctx)
                {
                    Mosfets = await (from cp in _ctx.Mosfet
        ...
        public async Task<vmMosfet> getbyid(int id)
        {
            ...
            try
            {
                using (_ctx)
                {
                    model = await (from cp in _ctx.Mosfet
                                                join p in _ctx.Project on cp.ProjectId equals p.Id
                                                join l in _ctx.Location on cp.LocationId equals l.Id
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            //Update Mosfet
                            var entityUpdate = _ctx.Mosfet.FirstOrDefault(x => x.Id == model.Id);
        ...
        public async Task<string> deletebyid(int id)
        {
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        var idToRemove = _ctx.Mosfet.SingleOrDefault(x => x.Id == id);
                        if (idToRemove != null)
                        {
                            _ctx.Mosfet.Remove(idToRemove);
                            await _ctx.SaveChangesAsync();
                        }
                        _ctxTransaction.Commit();
                        message = MessageConstants.Deleted;
