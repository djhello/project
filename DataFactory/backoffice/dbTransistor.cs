    ...
    public class dbTransistor
    {
        ...
        public async Task<List<vmTransistor>> getAll()
        {
            ...
            try
            {
                using (_ctx)
                {
                    Transistors = await (from cp in _ctx.dsTransistor
        ...
        public async Task<vmTransistor> getById(int id)
        {
            ...
            try
            {
                using (_ctx)
                {
                    model = await (from cp in _ctx.dsTransistor
                                                join p in _ctx.dsProject on cp.ProjectId equals p.Id
                                                join l in _ctx.dsLocation on cp.LocationId equals l.Id
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            //Update Transistor
                            var entityUpdate = _ctx.dsTransistor.FirstOrDefault(x => x.Id == model.Id);
                        ...
                        else
                        {
                           var TransistorModel = new Transistor
                           {
                            ...
                            _ctx.dsTransistor.Add(TransistorModel);
                            await _ctx.SaveChangesAsync();
                        }
        ...
        public async Task<string> deleteById(int id)
        {
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        var idToRemove = _ctx.dsTransistor.SingleOrDefault(x => x.Id == id);
                        if (idToRemove != null)
                        {
                            _ctx.dsTransistor.Remove(idToRemove);
                            await _ctx.SaveChangesAsync();
                        }
                        _ctxTransaction.Commit();
                        message = MessageConstants.Deleted;
