...
namespace DataFactory.backoffice
{
    public class dbOtherIC
    {
        ...
        {
            vmOtherIC model = null;

            try
            {
                using (_ctx)
                {
                    model = await (from cp in _ctx.dsOtherIC
                                                join p in _ctx.dsProject on cp.ProjectId equals p.Id
                                                join l in _ctx.dsLocation on cp.LocationId equals l.Id
        ...
        public async Task<string> create(OtherIC model)
        {
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            //Update OtherIC
                            var entityUpdate = _ctx.dsOtherIC.FirstOrDefault(x => x.Id == model.Id);
                        ...
                        else
                        {
                           var OtherICModel = new OtherIC
                           {
                            ...
                            _ctx.dsOtherIC.Add(OtherICModel);
                            await _ctx.SaveChangesAsync();
                        }
        ...
        public async Task<string> updateStatus(OtherIC model)
        {
            string message = string.Empty;
        ...
        public async Task<string> receive(vmOtherIC model)
        {
            string message = string.Empty;
        ...
        public async Task<string> deleteById(int id)
        {
            ...
            using (_ctx)
            {
                using (var _ctxTransaction = _ctx.Database.BeginTransaction())
                {
                    try
                    {
                        var idToRemove = _ctx.dsOtherIC.SingleOrDefault(x => x.Id == id);
                        if (idToRemove != null)
                        {
                            _ctx.dsOtherIC.Remove(idToRemove);
                            await _ctx.SaveChangesAsync();
                        }
                        _ctxTransaction.Commit();
                        message = MessageConstants.Deleted;
